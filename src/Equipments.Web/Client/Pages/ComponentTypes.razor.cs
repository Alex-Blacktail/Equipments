...
namespace Equipments.Web.Client.Pages
{
    public partial class ComponentTypes
    {
        ...
        private async Task GridLoadData(LoadDataArgs args)
        {
            try
            {
                types = await httpClient.GetFromJsonAsync<SomeTypeDto[]>(Routing.ComponentTypes);        
                count = types.Count();
            }
        ...
        {
            await DialogService.OpenAsync<AddComponentType>("Добавить тип комплектующего", null);
            await grid.Reload();
        }

        private async Task EditRow(SomeTypeDto args)
        {
            await DialogService.OpenAsync<EditComponentType>("Редактировать тип комплектующего", new Dictionary<string, object> { {"id", args.Id} });
            await grid.Reload();
        }
        ...
        private async Task GridDeleteButtonClick(MouseEventArgs args, SomeTypeDto type)
        {
            try
            {
                ...
                {
                    var deleteResult = await httpClient.DeleteAsync(Routing.ComponentTypes + type.Id);
