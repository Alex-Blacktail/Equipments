...
namespace Equipments.Web.Client.Pages
{
    public partial class EquipmentTypes
    {
        ...
        private async Task GridLoadData(LoadDataArgs args)
        {
            try
            {
                types = await httpClient.GetFromJsonAsync<SomeTypeDto[]>(Routing.EquipmentTypes);        
                count = types.Count();
            }
        ...
        {
            await DialogService.OpenAsync<AddEquipmentType>("Добавить тип оргтехники", null);
            await grid.Reload();
        }

        private async Task EditRow(SomeTypeDto args)
        {
            await DialogService.OpenAsync<EditEquipmentType>("Редактировать тип оргтехники", new Dictionary<string, object> { {"id", args.Id} });
            await grid.Reload();
        }
        ...
        private async Task GridDeleteButtonClick(MouseEventArgs args, SomeTypeDto type)
        {
            try
            {
                ...
                {
                    var deleteResult = await httpClient.DeleteAsync(Routing.EquipmentTypes + type.Id);
