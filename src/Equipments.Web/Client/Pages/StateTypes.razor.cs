...
namespace Equipments.Web.Client.Pages
{
    public partial class StateTypes
    {
        ...
        private async Task GridLoadData(LoadDataArgs args)
        {
            try
            {
                types = await httpClient.GetFromJsonAsync<SomeTypeDto[]>(Routing.StateTypes);        
                count = types.Count();
            }
        ...
        {
            await DialogService.OpenAsync<AddStateType>("Добавить тип состояния", null);
            await grid.Reload();
        }

        private async Task EditRow(SomeTypeDto args)
        {
            await DialogService.OpenAsync<EditStateType>("Редактировать тип состояния", new Dictionary<string, object> { {"id", args.Id} });
            await grid.Reload();
        }
        ...
        private async Task GridDeleteButtonClick(MouseEventArgs args, SomeTypeDto type)
        {
            try
            {
                ...
                {
                    var deleteResult = await httpClient.DeleteAsync(Routing.StateTypes + type.Id);
